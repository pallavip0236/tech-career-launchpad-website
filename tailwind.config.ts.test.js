...
function testTailwindConfig() {
  let success = true;
  let message = 'tailwind.config.ts should contain Tailwind CSS configuration';

  if (!tailwindConfigContent.includes('import type { Config } from "tailwindcss";')) {
    success = false;
  ...
  if (!tailwindConfigContent.includes('content: [')) {
    success = false;
    message += '\n  - Missing content array';
  }
  if (!tailwindConfigContent.includes("'./src/**/*.{ts,tsx}',")) {
    success = false;
    message += '\n  - Missing src content path';
  }
